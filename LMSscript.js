...
function addBook(book) {
  ...
  table.append(`
    ...
    const bookTitleSelect = $("#bookTitle");
    bookTitleSelect.append(`<option value="${book.title}" data-genre="${book.genre}">${book.title}</option>`);
...
function checkSelection() {
  ...
  var addBookButton = document.getElementById("addBtn");

  if (selectedValue === "addBook") {
    var bookTitle = prompt("Enter the title of the new book:");

    if (bookTitle !== null && bookTitle.trim() !== "") {
      var authorValue = prompt("Enter the author of the new book:");
      var yearValue = prompt("Enter the publication year of the new book:");
      ...
      newOption.setAttribute("data-author", authorValue);
      newOption.setAttribute("data-year", yearValue);
    ...
    // Disable the year and author fields for all options
    var options = selectElement.options;
    for (var i = 0; i < options.length; i++) {
      options[i].removeAttribute("data-author");
      options[i].removeAttribute("data-year");
    }
    ...
    // Enable the year and author fields only for the last option
